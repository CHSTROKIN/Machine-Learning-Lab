...
class neuro:
    ...
    def segmoid(self,n):#激活函数
        return (float)(1/(float)(1+math.exp(-n)))
    def get_input(self,the_input_list=[]):#更改训练集
        self.the_train_set=the_input_list
        return
    def error_func(self,y_pred,y):
        return (y-y_pred)*(y-y_pred)
    def gradient_numeric(self,variable_number):
        pass 
    def train(self,train_cycle):#用梯度下降法
        for i in tqdm(range(0,train_cycle)):#训练次数
            for j in range(0,len(self.the_train_set_x)):#遍历样本集合
               ...
               # mis=the_ret_value-one_correct_answer#误差
                mis=self.error_func(the_ret_value,one_correct_answer)
                for k in range(0,len(self.the_weight)):#遍历权重集合
                    self.the_weight[k]=self.the_weight[k]-self.learning_rate*2*(the_ret_value-one_correct_answer)*the_ret_value*(1-the_ret_value)*one_train_example[k]
                    self.the_constant=self.the_constant-self.learning_rate*2*(the_ret_value-one_correct_answer)*the_ret_value*(1-the_ret_value)
    ...
    def get_value(self,one_input_value):#返回lineage回归的输出
        z=np.dot(one_input_value,self.the_weight)#不要搞反了，矩阵的维度
        a=np.sum(z)
        a+=self.the_constant
        return (float)(self.segmoid(a))
def func(n):
    result=[]
    i=2
    end=0
...
if __name__ == '__main__':
    ...
    for i in range(0,1000):
        y.append((float)(1/(float)(1+math.exp(-i*2000))))
        value=[]
        value.append(i)
        x.append(value)
       # print(x[i],' ',y[i])
    my_neuro=neuro(0.01,1,x,y)
    ...
    my_neuro.print_weight()
    print(my_neuro.get_value(1))
    print((float)(1/(float)(1+math.exp(-1*2000))))
   # print(t2-t1)